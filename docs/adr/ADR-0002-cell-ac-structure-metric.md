...
| ours Q3 − chafa (재구성 리드) | **+0.0036** (d=1.14) | **+0.0191** (d=3.29) | **+0.0211** (d=2.16) | CAS가 SSIM이 압축한 구조 마진을 해상 |
| Q4 − Q3 (SSIM이 구분 못 한 쌍) | +0.0001 (잡음) | −0.0008 | −0.0008 | 아래 정직 주석 |

- **de-saturation 입증(정정 2026-07-12, fix/cas-multiplier-claim)**: 종전의 "SSIM 대비 5–6× 크게
  해상"(wmean 5.3×, p05 5.9×) 배수 주장은 **철회** — raw 마진의 cross-metric 비율은 척도 의존이라
  (CAS를 `a·CAS+b`로 affine 재표기만 해도 배수가 a배로 바뀜) 진술된 수치가 단위 선택의 인공물이었다.
  척도 불변 재서술: 이미지별 마진의 표준화 효과크기 **d = mean/std**(표본 std n−1, n=6 소표본 —
  정성적으로 읽을 것)로 wmean **d=3.29**, p05 **d=2.16** vs SSIM **d=1.14**. 정성적 포화 주장은 유지 —
  SSIM은 자기 천장(≈0.95) 근처 소수점 3–4자리로 리드를 압축하고 CAS는 분리하며, 척도와 무관한 근거는
  아래 Q4−Q3의 부호 일관성이다. CAS median(p50)은 d=0.26로 SSIM처럼 포화 — 그래서 headline은 중앙값이
  아니라 **하위 percentile + 에너지 가중**이다(설계와 정합).
- **Q3 vs Q4 정직 주석**: 이 매끈한 렌더에선 Q3와 Q4가 **진짜로 근접**하다(Q4 edge loss가 물 곳이 거의
  ...
  부호 뒤집힘)과 달리 CAS는 **일관 부호의 −0.0008**(6/6 이미지 동일 부호, Q4가 내부 AC를 미세 교환 —
  부호 일관성은 척도 무관) — 신호를 해상하되
