...
- **수용 판정(논리곱)**: (a) 블라인드 A/B 다수승 ∧ (b) ASCII-identity 프록시가 사전 등록 마진만큼
...
- **미학 태스크를 CAS/재구성으로 판정** — selection-prior 정리에 정면 위배(prior는 재구성을 못 이김).
  미학은 별도 목적 함수 + 블라인드 시각 판정 + 양면 가드레일로 판정하도록 확정.
